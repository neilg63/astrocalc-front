...
import { TransitList, TransitItem } from "~/api/models";
...
export default function TransitListTable({ data }: { data: TransitList  }) {
  ...
  const toTabGrid = () => {
    const rows = data.rows().map((row: TransitItem[]) => {
      return row.map((cell:TransitItem) => julToISODate(cell.jd, tzOffset)).join("\t");
    });
  ...
  const cellClasses = (cell: TransitItem, index = -1) => {
    const cls = [];
    if (cell.valid) {
      cls.push(cell.key);
            ...
            {(cell, cellIndex) => <>
              <td class={cellClasses(cell, cellIndex())}>
                <Show when={cell.valid}>
                  <div class="column">
                    <div class="flex flow-row transit-type">
