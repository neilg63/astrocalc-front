...
import { SunTransitList, TransitSet } from "~/api/models";
...
export default function SunTransitListTable({ data }: { data: SunTransitList }) {
  ...
  const toTabGrid = () => {
    const rows = data.items.map((row: TransitSet) => {
  ...
  const rowClasse = (cell: TransitSet, index = -1): string => {
